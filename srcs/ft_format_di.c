...
/*   Updated: 2019/08/08 18:04:57 by mfahey           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_format_di_scnd(t_printf *ft)
{
	if (ft->size == hh)
	{
		...
		ft->value->s = ft_llitoa_base(ft->value->hhdi, 10);
	}
	else if (ft->size == h)
	{
		...
		ft->value->s = ft_llitoa_base(ft->value->hdi, 10);
	}
}
...
void	ft_format_di_frst(t_printf *ft)
{
	if (ft->size == l)
	{
		...
		ft->value->s = ft_llitoa_base(ft->value->ldi, 10);
	}
	else if ((ft->size == ll) || (ft->size == j))
	{
		...
		ft->value->s = ft_llitoa_base(ft->value->lldi, 10);
	}
	else
...
void	ft_format_di(t_printf *ft)
{
	if (ft->size == ws)
	{
		...
		ft->value->s = ft_llitoa_base(ft->value->di, 10);
	}
	else if (ft->size == z)
	{
	...
	if (ft->pn)
		(ft->k)++;
	ft->length = ft_strlen(ft->value->s) - ft->pn;
